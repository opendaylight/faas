...
public class RouterListener implements DataChangeListener, AutoCloseable {
    ...
    public RouterListener(DataBroker dataProvider, ScheduledExecutorService executor) {
        this.executor = executor;
        this.registerListener = checkNotNull(dataProvider).registerDataChangeListener(LogicalDatastoreType.OPERATIONAL, UlnIidFactory.logicalRouterIid(), this,
    ...
    public void executeEvent(AsyncDataChangeEvent<InstanceIdentifier<?>, DataObject> change) {
        ...
        for (DataObject dao : change.getCreatedData().values()) {
            if (dao instanceof LogicalRouter) {
                LOG.debug("Created Logical Router {}", (LogicalRouter) dao);
            }
        }
        // Update
        ...
        for (Map.Entry<InstanceIdentifier<?>, DataObject> entry : dao.entrySet()) {
            if (entry.getValue() instanceof LogicalRouter) {
                LOG.debug("Updated Logical Router {}", (LogicalRouter) dao);
            }
        }
        // Remove
        for (InstanceIdentifier<?> iid : change.getRemovedPaths()) {
            ...
            if (old instanceof LogicalRouter) {
                LOG.debug("Removed Logical Router {}", (LogicalRouter) old);
            }
        }
    }
