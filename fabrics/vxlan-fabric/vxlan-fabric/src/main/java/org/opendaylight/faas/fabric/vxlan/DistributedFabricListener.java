...
//                addTp2Fabric(device.getNodeId(), device.getTerminationPoint());
    ...
    @Override
    public void portFuncUpdate(InstanceIdentifier<PortFunction> iid, PortFunction function, boolean delete) {
        InstanceIdentifier<TerminationPoint> tpiid = iid.firstIdentifierOf(TerminationPoint.class);
        NodeId nodeId = iid.firstKeyOf(Node.class).getNodeId();
        if (fabricCtx.isValidLogicSwitch(nodeId)) {
        ...
        //fabricCtx.getLogicRouterCtx(nodeId).getGatewayPort(ip);
    }
    ...
    @Override
    public void routeUpdate(InstanceIdentifier<Node> lrIid, List<Route> routes, boolean delete) {
