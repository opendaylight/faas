...
import org.opendaylight.yang.gen.v1.urn.opendaylight.faas.fabric.type.rev150930.logic.port.UnderlayerPorts;
import org.opendaylight.yang.gen.v1.urn.opendaylight.faas.fabric.type.rev150930.logic.port.UnderlayerPortsBuilder;
import org.opendaylight.yang.gen.v1.urn.opendaylight.faas.fabric.type.rev150930.logic.port.UnderlayerPortsKey;
...
public class EpAccessPortRenderer {
	...
	public void createEpAccessPort(WriteTransaction trans, Endpoint ep, TpId bridgeDomainPort) throws Exception {
		...
		InstanceIdentifier<TerminationPoint> underlayerTpIid = underlayernode.child(TerminationPoint.class, new TerminationPointKey(bridgeDomainPort));

		Optional<TerminationPoint> optional = readTp();

		InstanceIdentifier<UnderlayerPorts> iid = lPortIid.augmentation(LogicPortAugment.class)
		...
		if (optional.isPresent()) {
			LogicPortAugment lpAug = optional.get().getAugmentation(LogicPortAugment.class);
			if (lpAug != null) {
	...
	private boolean calcLogicPortIId(Endpoint ep) {
		LogicLocation ll = ep.getLogicLocation();
		if (ll == null) {
			return false;
		}
