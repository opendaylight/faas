...
public class Openflow13Provider {
    ...
    public void updateBridgeDomainAclsInDevice(Long dpid, Long segmentationId, FabricAcl fabricAcl, boolean writeFlow) {
        ...
        InstanceIdentifier<Acl> aclIID = InstanceIdentifier.create(AccessLists.class).child(Acl.class,
                new AclKey(ietfAclName, Ipv4Acl.class));
    ...
    public void updateBridgeDomainPortInDevice(Long dpid, Long ofPort, Long segmentationId,
            BridgeDomainPort bridgeDomainPort, boolean writeFlow) {
        AccessType accessType = bridgeDomainPort.getAccessType();
        if (accessType.getIntValue() == AccessType.Vlan.getIntValue()) {
            if (bridgeDomainPort.getAccessTag() != null) {
                trafficClassifier.programVlanInPort(dpid, bridgeDomainPort.getAccessTag(), segmentationId, ofPort,
                        writeFlow);
            }
        }
    }
