...
public class FabricDeviceManager implements FabricVxlanDeviceAdapterService, DataChangeListener, AutoCloseable {
    ...
    public Future<RpcResult<Void>> addToVxlanFabric(final AddToVxlanFabricInput input) {
        ...
        addTp2Fabric(wt, bridgeNode, deviceIId);
    ...
    private void addTp2Fabric(WriteTransaction wt, Node bridgeNode, InstanceIdentifier<Node> deviceIId) {
        NodeBuilder builder = new NodeBuilder();
        builder.setKey(bridgeNode.getKey());
        ...
        for (TerminationPoint tp : bridgeNode.getTerminationPoint()) {
            ...
            FabricPortAug aug = new FabricPortAugBuilder().setPortRole(FabricPortRole.Access).build();
            tpBuilder.addAugmentation(FabricPortAug.class, aug);

            updTps.add(tpBuilder.build());
        }
        builder.setTerminationPoint(updTps);
        wt.merge(LogicalDatastoreType.OPERATIONAL, deviceIId, builder.build(), false);
    }
