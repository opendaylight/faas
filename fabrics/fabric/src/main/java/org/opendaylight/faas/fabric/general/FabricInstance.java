...
public class FabricInstance implements FabricRenderer {
    ...
    public void notifyPortFuncUpdate(InstanceIdentifier<PortFunction> iid, PortFunction function, boolean delete) {
        for (FabricListener listener : listeners) {
            listener.portFuncUpdate(iid, function, delete);
        }
    }

    public void notifyRouteUpdate(InstanceIdentifier<Node> lrIid, List<Route> routes, boolean delete) {
        for (FabricListener listener : listeners) {
            listener.routeUpdate(lrIid, routes, delete);
        }
    }
